...
export const shopifyNumberToHumanPrice = (
  ...
  let shopifyPrice = Number.isInteger(shopifyNumber)
    ? shopifyNumberToShopifyPrice(shopifyNumber)
    : shopifyNumber;
...
export function makeCurrency(price, locale = 'en-US', currencyCode = 'USD') {
  let currencyLocale = checkLocale(toBCP47LocaleTag(locale), currencyCode);
...
export function replaceMoneyByCurrency(str, currencyCode = 'USD', valueMap) {
  ...
  let finalStr = str;
  const currencyValue =
    str.match(/%%value%%/) || str.match(/[$|€|£]+ ?[0-9]+\.?[0-9]{2}?/);
  const newValue = valueMap[currencyCode];

  if (currencyValue && newValue) {
    const [value] = currencyValue;
    const formatterValue = makeCurrency(
      newValue,
      currencyCode === 'EUR' ? 'nl-NL' : 'en-US',
      currencyCode
    );
